...
function Wait-Ready {
    for ($attempt = 0; $attempt -lt 40; $attempt++) {
        try {
            $response = Invoke-WebRequest -Headers (Get-PokemonAuthorizationHeaders) "$BaseUrl/health/ready" -TimeoutSec 10 -SkipHttpErrorCheck
...
Wait-Ready
$battle = Invoke-RestMethod -Headers (Get-PokemonAuthorizationHeaders) "$BaseUrl/battles" -Method Post -ContentType application/json -Body (@{
...
$battle = Invoke-RestMethod -Headers (Get-PokemonAuthorizationHeaders) "$BaseUrl/battles/$($battle.id)/turns" -Method Post -ContentType application/json -Body ($turn | ConvertTo-Json)
...
Wait-Ready
$recovered = Invoke-RestMethod -Headers (Get-PokemonAuthorizationHeaders) "$BaseUrl/battles/$($battle.id)"
...
$continued = Invoke-RestMethod -Headers (Get-PokemonAuthorizationHeaders) "$BaseUrl/battles/$($battle.id)/turns" -Method Post -ContentType application/json -Body ($turn | ConvertTo-Json)
