...
/// <summary>Consulta y persiste movimientos mediante Dapper en la transacción de la unidad de trabajo.</summary>
internal sealed class MoveRepository(CatalogDatabaseSession session) : IMoveRepository, IMoveReader
{
    ...
    /// <summary>Representa los datos de una fila de movimientos antes de reconstruir el agregado.</summary>
    private sealed record Row(Guid Id, string Name, int Power, int Type)
    {
        /// <summary>Reconstruye el movimiento y trata los datos almacenados inválidos como un error de persistencia.</summary>
        public CatalogMove ToDomain() => DomainMaterializer.Create(() => new CatalogMove(Id, Name, Power, (PokemonType)Type));
    }

    /// <summary>Ejecuta un filtro SQL interno parametrizado y reconstruye los movimientos encontrados.</summary>
    ...
    /// <summary>Busca un movimiento por su identificador y devuelve null si no existe.</summary>
