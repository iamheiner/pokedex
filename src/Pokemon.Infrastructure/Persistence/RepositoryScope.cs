...
/// <summary>Crea y reutiliza bajo demanda una instancia de cada repositorio durante una operación transaccional.</summary>
internal sealed class RepositoryScope(DatabaseSession database, RepositoryRegistry registry, bool readOnly) : IRepositoryScope
{
    ...
    /// <summary>Obtiene el lector solicitado y reutiliza su instancia durante la operación actual.</summary>
    public TReader GetReader<TReader>() where TReader : class, IReadRepository => Resolve<TReader>();

    /// <summary>Obtiene el repositorio de escritura solicitado y rechaza su resolución en una sesión de lectura.</summary>
    ...
    /// <summary>Resuelve el contrato registrado y crea su implementación únicamente en el primer acceso.</summary>
