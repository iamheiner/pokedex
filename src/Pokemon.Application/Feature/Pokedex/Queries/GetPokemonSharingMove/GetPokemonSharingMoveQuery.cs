...
public sealed class GetPokemonSharingMoveQueryHandler(IPokedexReadSession reader) : IRequestHandler<GetPokemonSharingMoveQuery, IReadOnlyList<PokemonView>>
{
    public Task<IReadOnlyList<PokemonView>> Handle(GetPokemonSharingMoveQuery request, CancellationToken token) =>
        reader.ReadAsync<IReadOnlyList<PokemonView>>(async data =>
        {
            ...
            return await PokedexMapping.ViewsAsync(data, await data.Pokemon.FindByMoveAsync(request.Id, new CatalogPage(request.Offset, request.Limit), token), token);
        }, token);
}
