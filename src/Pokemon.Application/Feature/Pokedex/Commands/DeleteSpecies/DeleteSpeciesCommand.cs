...
public sealed class DeleteSpeciesCommandHandler(IPokedexUnitOfWork store) : IRequestHandler<DeleteSpeciesCommand, Unit>
{
    public Task<Unit> Handle(DeleteSpeciesCommand request, CancellationToken token) =>
        store.Write(data =>
        {
            ...
            data.SpeciesRepository.Delete(request.Id);
            return Unit.Value;
        }, token);
}
