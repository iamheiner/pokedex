...
public sealed record SpeciesView(Guid Id, string Name, PokemonType Type, StatsInput Stats, IReadOnlyList<LearningView> Learnset);
public sealed record PokemonView(Guid Id, Guid SpeciesId, string SpeciesName, string Name, PokemonType Type,
    int Level, int CurrentHealth, int TotalHealth, StatsInput Stats, IReadOnlyList<MoveView> Moves);
